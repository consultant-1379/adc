...
def collectJson(servername,host,metric,api,step,logdir):
    ...
    with open(f"{logdir}/pm_metrics/{servername}_{metric}.json","w") as file:
        file.write(result)
...
def collectVictoriaNative(servername,host,metric,api):
    global start_timestamp
    global end_timestamp
    command=f"curl -ks https://{host}{api}export/native  -d \'match[]={metric}\' -d \"start={start_timestamp}\" -d \"end={end_timestamp}\"  > {logdir}/pm_metrics/{servername}_{metric}.bin"    
...
def collectSingleValue(host,query,api):
    ...
    out, err = process_command.communicate()
    if out == None or len(out)==0 or out==b'null\n':
...
def critPodTable(servername,critPodsList,server,logdir):
    ...
    with open(f"{logdir}/critPodTable_{servername}.csv","w") as file:
        file.write(critPodTable)

        
def pcgDpPodTable(servername,roles,host,api,logdir):
    pcgDpPod=""
    #  pcg DP pod CPU
    ...
    with open(f"{logdir}/pcgDpPod_{host}.csv","w") as file:
        file.write(pcgDpPod)
...
start_timestr=args.starttime
end_timestr=args.endtime

start_timestamp = str(int(datetime.timestamp(datetime.strptime(start_timestr, "%Y-%m-%d %H:%M:%S"))))
end_timestamp = str(int(datetime.timestamp(datetime.strptime(end_timestr, "%Y-%m-%d %H:%M:%S") )))
timerange=str(int(end_timestamp)-int(start_timestamp)) 


pm_metrics_folder= f"{logdir}/pm_metrics"
...
for servername in configDict["servers"]:
    ...
    if server["pmtype"]=="victoria-metrics":
        for metric in server.get("metrics",[]):
            ...
            collectJson(servername,server["host"],metric,server["api"],"1h",logdir)
    elif server["pmtype"]=="prometheus":
        for metric in server.get("metrics",[]):
            collectJson(servername,server["host"],metric,server["api"],"5m",logdir)
    else: 
        print(f"pmtype  not recognised")
    if "criticalPodsTable" in server:
        critPodTable(servername,critPodsList,server,logdir)
    #if "dpPodtable" in server:
    #    pcgDpPodTable(servername,server["dpPodtable"],server["host"],server["api"],logdir)
