...
from utils import get_active_alerts


def save_to_file(actalarm, actalerts, cluster, logdir):
    """save alarms log to logfile"""
    if not os.path.exists(logdir):
        os.makedirs(logdir)
    for ns, alarms in _parse_alarms(actalarm).items():
        logfile = os.path.join(logdir, f'{ns}-{cluster}.alarms')
        for alarm in alarms:
            ...
            with open(logfile, 'a+') as f:
               f.write(alarm)
               f.write('\n')
    logfile = os.path.join(logdir, f'{cluster}.alerts')
    with open(logfile, 'a+') as f:
        for _, als in _parse_alarms(actalerts, type='alert').items():
            for al in als:
                f.write(al)
                f.write('\n')


def _parse_alarms(content, type='alarm'):
    result = dict()
    ...
    if type == 'alert':
        ...
        if content["data"].get('alerts'):
            for aal in content["data"]["alerts"]:
                result.setdefault("alerts", []).append(
                    ' - '.join([timestamps, str(aal)])
                    )
        return result
    for n in content:
        ns = n["namespace"]
...
def main(input_args):
    ...
    if options.save_alarms:
        ...
        act_alerts = get_active_alerts(options.pod, options.cluster)
        log.info(json.dumps(outputs))
        save_to_file(act_alarms, act_alerts, options.cluster, logdir)
        log.info('All active 5GC alarms and eccd alerts has been saved to "{0}".'
            .format(logdir))
