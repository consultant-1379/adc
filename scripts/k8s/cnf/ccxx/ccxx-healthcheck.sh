#-------------------------------------------------------------------------------------------
# Description: The script is for CCXX Healthcheck on ECCD Traffic Cluster Director server.
...
##############################################################################
########### run the script on Traffic Cluster Director for CCXX Healthcheck###
##############################################################################
VERSION=1.2

# 0: don't collect pod log when ccxx in unhealthy state
# 1: Collect pod log when ccxx in unhealthy state 
...
function usage() {
  echo;
  log_info 'Description: Run it on eccd cluster for CCXX HealthCheck.'
  log_info 'usage: ./ccxx_healthcheck.sh <namespace> <testing Description>'
  log_info 'usage:Example: ./ccxx_healthcheck.sh ccdm or ./ccxx_healthcheck.sh ccdm "before work node reboot"'
  echo;
}
...
function check_alarms() {
  ...
  then 
    ...
    CCXX_NS=$1
    alarms_list=$(kubectl -n ${CCXX_NS} exec -it $(kubectl -n ${CCXX_NS} get pods | grep fh-alarm | grep -v topics | tail -1 | awk '{print $1}') -c eric-fh-alarm-handler -- curl -k --cert /run/secrets/client-cert/clicert.pem --key /run/secrets/client-cert/cliprivkey.pem https://eric-fh-alarm-handler:5006/ah/api/v0/alarms?outputFormat=FullAlarmList | jq . |tee -a $REPORT_FILE )

    #echo "### check_alarms ${CCXX_NS}-->alarm_list:${alarms_list} "
    len=$(echo ${alarms_list} | awk '{print length($0)}') 
    
    # check alarm list is empth or not ,and set the return value
    if [ ${len} -gt 2 ] 
    then 
      ret=1
      log_error "$alarms_list"
    else
      log_info "### check_alarms $CCXX_NS : No alarm,Good!" |tee -a $REPORT_FILE
      ret=0
    fi
  fi
