...
help()
{
    echo "This script collect memory log in the vnf."
...
logname='$i'"pod_memory"`date -d @$endtime +"%Y-%m-%d_%H-%M-%S" `.txt

curl_savefile () {
  curl  http://monitoring-eric-victoria-metrics-cluster-vmselect.ingress.$evnfm-$cluster.sero.gic.ericsson.se/select/0/prometheus/api/v1/export -d 'match=container_memory_usage_bytes{name!="",namespace="'$i'",container=~".*"}' -d 'start='$starttime'' -d 'end='$endtime''|jq . >> $logname
}

filename="pod_memory"`date -d @$endtime +"%Y-%m-%d_%H-%M-%S" `
mkdir $filename
cd $filename
